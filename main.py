...
def search(term):
    # Search for products with names or descriptions containing the given term (case-insensitive).
...
def validate_product_data(product_data):
    ...
    if product_data["price"] < 0 or product_data["quantity"] < 0:
        raise ValueError(
            "Product price and quantity must be positive numbers.")


class WhooshIndex:
    def __init__(self, index_dir):
        # Initialize the Whoosh index for product search.
    ...
    def search(self, term):
        # Search for products using the Whoosh index.
        with self.index.searcher() as searcher:
            query_parser = MultifieldParser(
                ["name", "description"], self.index.schema)
...
def populate_test_database():
    ...
    user1 = User.create(name="Alice", address="123 Wonderland St",
                        billing_info="Visa 9876")
    user2 = User.create(
        name="Bob", address="456 Dreamland Ave", billing_info="Mastercard 5432"
    )
    user3 = User.create(
        name="Charlie", address="789 Fantasia Blvd", billing_info="American Express 9876"
    )
    ...
    product4 = Product.create(
        name="Jacket",
        description="Cool and versatile",
        price=129.99,
        quantity=8,
        owner=user2,
    )
    ...
    tag3, _ = Tag.get_or_create(name="Fashion")
    ...
    Transaction.create(buyer=user1, product=product2, quantity=3)
